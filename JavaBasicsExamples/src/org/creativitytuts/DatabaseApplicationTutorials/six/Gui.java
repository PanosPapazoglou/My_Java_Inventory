	...
	public void btnAction(){
		...
		nr.addActionListener(new ActionListener(){ 
			public void actionPerformed(ActionEvent ev){
				...
				// Set all text Field to null - Clean text Fields
				t.setText("");
				t1.setText("");
				t2.setText("");
		...
		save.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent ev){
				// Katarxas gia na kanoume tin apothikeysi theloume na paroume se metablites ayta pou einai gramena sta textfields
				String fname = t.getText();
				String lname = t1.getText();
				String age = t2.getText();
				...
				try{
					// We will write the code to insert to database the data in the textfields
					//But First we need to move the cursor in the database to a new row
					// So to do this we have to write the following command 
					rs.moveToInsertRow();

					// Stin sinexeia grafoume tis entoles gia tin apothikeysi
					rs.updateString("Fname", fname);
					rs.updateString("Lname", lname);
					rs.updateString("Age", age);

					//Commit...Insert row to the database..
					rs.insertRow();
